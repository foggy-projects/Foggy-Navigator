# gemini-worker.ps1 -- Unified CLI for Gemini Agent Worker (Windows)
...
if ((Test-Path $MonorepoPackageJson) -and -not (Test-Path (Join-Path $InstallDir "VERSION"))) {
    try {
        ...
        if ($pkg.name -eq "gemini-agent-worker") {
...
if (Test-Path $EnvFile) {
    $portLine = Get-Content $EnvFile | Where-Object { $_ -match "^GEMINI_WORKER_PORT=(\d+)" }
    if ($portLine -and $portLine -match "=(\d+)") { $Port = [int]$Matches[1] }
}

function Invoke-Start {
    Write-Host "Starting Gemini Agent Worker v$Version..." -ForegroundColor Cyan
...
function Invoke-Version {
    Write-Host "gemini-worker $Version"
}
...
function Invoke-Upgrade {
    ...
    Write-Host "Gemini Agent Worker Upgrade" -ForegroundColor Cyan
    ...
    $workerUrl = $env:GEMINI_WORKER_URL
    if (-not $workerUrl -and (Test-Path $EnvFile)) {
        $urlLine = Get-Content $EnvFile | Where-Object { $_ -match "^GEMINI_WORKER_URL=(.+)" }
    ...
    Write-Host "  gemini-worker upgrade C:\path\to\gemini-worker-X.Y.Z-windows.zip"
    Write-Host ""
    Write-Host "To enable auto-upgrade, set:" -ForegroundColor Yellow
    Write-Host "  GEMINI_WORKER_URL   OBS/HTTP base URL (in .env or environment)"
}
...
function Invoke-UpgradeFromObs {
    ...
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "gemini-worker-upgrade"
...
function Invoke-UpgradeFromArchive {
    ...
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "gemini-worker-extract"
    if (Test-Path $tmpDir) { Remove-Item $tmpDir -Recurse -Force }
    New-Item -ItemType Directory -Force -Path $tmpDir | Out-Null
...
function Invoke-Help {
    Write-Host "gemini-worker v$Version -- Gemini Agent Worker CLI"
    Write-Host ""
    Write-Host "Usage: gemini-worker <command> [options]"
    Write-Host ""
    Write-Host "Commands:"
    ...
    Write-Host "  GEMINI_WORKER_HOME   Install directory (default: ~\.gemini-worker)"
    Write-Host "  GEMINI_WORKER_URL    OBS/HTTP base URL for auto-upgrade"
    Write-Host ""
