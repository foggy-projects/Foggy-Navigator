#!/bin/bash
# Gemini Agent Worker - Remote Bootstrap Installer (Linux / macOS)
...
echo -e "${CYAN}=== Gemini Agent Worker - Remote Installer ===${NC}"
echo ""
...
FILE_PATH=$(echo "$LATEST_JSON" | grep "\"$OS_TAG\"" | head -1 | sed 's/.*"\([^"]*gemini-worker[^"]*\)".*/\1/')
if [ -z "$FILE_PATH" ]; then
...
export GEMINI_WORKER_URL="$RELEASE_BASE_URL"
