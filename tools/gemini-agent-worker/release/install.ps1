# Gemini Agent Worker - Installer (Windows)
...
$InstallDir = if ($env:GEMINI_WORKER_HOME) { $env:GEMINI_WORKER_HOME } else { Join-Path $env:USERPROFILE ".gemini-worker" }
...
if ($IsUpgrade) {
    ...
    Write-Host "Upgrading Gemini Agent Worker: $OldVersion -> $Version" -ForegroundColor Cyan
}
else {
    Write-Host "Installing Gemini Agent Worker v$Version" -ForegroundColor Cyan
}
...
if (-not $geminiExists) {
    Write-Host "WARNING: Gemini CLI not found in PATH." -ForegroundColor Yellow
    Write-Host "The worker can still start, but subscription login mode requires 'gemini'." -ForegroundColor Yellow
    Write-Host ""
}
...
Copy-Item (Join-Path $ScriptDir "bin\gemini-worker.ps1") $BinDir -Force
if (Test-Path (Join-Path $ScriptDir "bin\gemini-worker")) {
    Copy-Item (Join-Path $ScriptDir "bin\gemini-worker") $BinDir -Force
}

$CmdShim = Join-Path $BinDir "gemini-worker.cmd"
@"
@echo off
powershell -ExecutionPolicy Bypass -File "%~dp0gemini-worker.ps1" %*
...
if ($env:GEMINI_WORKER_URL) {
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    $envContent = if (Test-Path $EnvPath) { [System.IO.File]::ReadAllText($EnvPath, $utf8NoBom) } else { "" }
    if ($envContent -match "(?m)^GEMINI_WORKER_URL=") {
        $envContent = $envContent -replace "(?m)^GEMINI_WORKER_URL=.*", "GEMINI_WORKER_URL=$($env:GEMINI_WORKER_URL)"
    }
    else {
        $envContent = $envContent.TrimEnd() + "`n`n# Auto-upgrade URL (set by remote installer)`nGEMINI_WORKER_URL=$($env:GEMINI_WORKER_URL)`n"
    }
    Write-Utf8File -Path $EnvPath -Content $envContent
    Write-Host "Saved GEMINI_WORKER_URL to .env" -ForegroundColor Green
}
...
if (-not $UserPath -or -not $UserPath.Contains($BinDir)) {
    Write-Host "To add 'gemini-worker' to your PATH, run:" -ForegroundColor Yellow
...
if ($IsUpgrade) {
    Write-Host "Gemini Agent Worker upgraded to v$Version" -ForegroundColor Green
}
else {
    Write-Host "Gemini Agent Worker v$Version installed!" -ForegroundColor Green
}
Write-Host "============================================" -ForegroundColor Green
Write-Host ""
Write-Host "Commands:"
Write-Host "  gemini-worker start     Start the worker" -ForegroundColor Cyan
Write-Host "  gemini-worker stop      Stop the worker" -ForegroundColor Cyan
Write-Host "  gemini-worker status    Check status & health" -ForegroundColor Cyan
Write-Host "  gemini-worker version   Show version" -ForegroundColor Cyan
Write-Host "  gemini-worker logs      Tail log output" -ForegroundColor Cyan
Write-Host "  gemini-worker upgrade   Upgrade to new version" -ForegroundColor Cyan
Write-Host ""
