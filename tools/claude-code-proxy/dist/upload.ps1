# Claude Code Proxy — Upload to Huawei Cloud OBS
# Uploads release archives + latest.json + bootstrap scripts to OBS.
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File dist\upload.ps1
#   powershell -ExecutionPolicy Bypass -File dist\upload.ps1 -Version 1.1.0
#   powershell -ExecutionPolicy Bypass -File dist\upload.ps1 -Version 1.1.0 -AllowSameVersion
#
# Prerequisites:
#   - obsutil installed and configured (obsutil config -i=AK -k=SK -e=endpoint)
#   - .env with RELEASE_OBS_BUCKET and RELEASE_BASE_URL (in proxy root)
...
# --- Load .env from proxy root -------------------------------------------
$DotEnv = Join-Path $ProxyDir ".env"
if (-not (Test-Path $DotEnv)) {
    Write-Host "ERROR: .env not found at $ProxyDir" -ForegroundColor Red
...
Write-Host "=== Claude Code Proxy — OBS Upload ===" -ForegroundColor Cyan
...
$archives = Get-ChildItem $OutputDir -File | Where-Object { $_.Name -like "claude-code-proxy-$Version-*" }
...
# --- Upload archives to obs://bucket/claude-code-proxy/{version}/ --------
...
Write-Host ""
Write-Host "Upgrade:      claude-code-proxy upgrade" -ForegroundColor White
