#!/bin/bash
# Claude Code Proxy — Remote Bootstrap Installer (Linux / macOS)
...
echo -e "${CYAN}=== Claude Code Proxy — Remote Installer ===${NC}"
echo ""
...
FILE_PATH=$(echo "$LATEST_JSON" | grep "\"$OS_TAG\"" | head -1 | sed 's/.*"\([^"]*claude-code-proxy[^"]*\)".*/\1/')
if [ -z "$FILE_PATH" ]; then
...
# Pass through CLAUDE_PROXY_URL so install.sh can write it to .env
export CLAUDE_PROXY_URL="$RELEASE_BASE_URL"
