# Claude Code Proxy — Remote Bootstrap Installer (Windows)
...
Write-Host "=== Claude Code Proxy - Remote Installer ===" -ForegroundColor Cyan
Write-Host ""
...
$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "claude-code-proxy-remote-install"
...
# Pass through CLAUDE_PROXY_URL so install.ps1 can write it to .env
$env:CLAUDE_PROXY_URL = $ReleaseBaseUrl
