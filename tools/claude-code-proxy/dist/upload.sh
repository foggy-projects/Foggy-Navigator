...
# Usage:
#   bash dist/upload.sh
#   bash dist/upload.sh 1.1.0    # Specify version explicitly
#   bash dist/upload.sh 1.1.0 --allow-same-version
#
# Prerequisites:
...
DOT_ENV="$PROXY_DIR/.env"
if [ ! -f "$DOT_ENV" ]; then
    echo -e "${RED}ERROR: .env not found at $PROXY_DIR${NC}"
...
echo -e "${CYAN}=== Claude Code Proxy — OBS Upload ===${NC}"
...
ARCHIVES=$(find "$OUTPUT_DIR" -maxdepth 1 -name "claude-code-proxy-${VERSION}-*" -type f 2>/dev/null)
if [ -z "$ARCHIVES" ]; then
...
echo ""
echo -e "  Upgrade:    claude-code-proxy upgrade"
