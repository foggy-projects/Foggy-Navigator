...
#   powershell -ExecutionPolicy Bypass -File dist\package.ps1 -OS all -Version 0.1.4 -Upload
...
function Set-WorkerVersion {
    ...
    if ($content -notmatch '__version__\s*=\s*"([^"]+)"') {
        Write-Host "ERROR: Could not update version in src/agent_worker/__init__.py" -ForegroundColor Red
        exit 1
    }
...
elseif ($Bump -ne "none") {
    $currentVersion = Get-WorkerVersion
...
function Build-ForOS {
    ...
    $ArchiveName = "claude-worker-$ReleaseVersion-$OsTag"
