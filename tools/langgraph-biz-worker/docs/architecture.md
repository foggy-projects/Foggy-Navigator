...
| 完成方式 | 自由文本输出 | Root 可自然语言完成；结构化 frame 状态用 `submit_frame_result` |
...
Root 普通回合可以用自然语言直接完成，BizWorker 会提交当前 turn 的 runtime memory、report 和 SSE result。`submit_frame_result` 是 frame-oriented runtime control tool：Root 只有在需要保存 `active_plan`、`artifact_refs`、`evidence_refs`、`structured_output` 等结构化状态时才调用；non-root Agent frame 完成、等待用户补充或提交 refs 时优先调用。`submit_skill_result` 仅作为旧名兼容别名保留。

结构化 frame result 流程：

```
模型调用 submit_frame_result(summary, structured_output, ...)
...
- LLM 决策调用工具 → 自然语言最终答复，或在需要结构化状态时调用 `submit_frame_result`
