...
class FrameExecutionReportGenerator:
    ...
    def _build_digest(
        ...
        return {
            ...
            "turn_status": report_status,
            "report_status": report_status,
            "persistent_frame_status": frame.status.value,
    ...
    def _build_markdown(
        ...
        lines = [
            ...
            f"- turn_status: `{_md_text(digest['turn_status'])}`",
            f"- persistent_frame_status: `{_md_text(digest['persistent_frame_status'])}`",
            f"- status: `{_md_text(digest['status'])}`",
            f"- frame_status: `{_md_text(frame.status.value)}`",
            "- status_note: `status` is a compatibility alias for turn/report status; "
            "`frame_status` is a compatibility alias for persistent frame lifecycle status.",
...
def _child_report_digest(digest: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "turn_status": digest.get("turn_status"),
        "report_status": digest.get("report_status"),
        "persistent_frame_status": digest.get("persistent_frame_status"),
