...
export async function bootstrap(): Promise<{ manager: TaskManager; close: () => Promise<void> }> {
  const stateLease = await acquireStateWriterLease(config.stateDir)
  let manager: TaskManager | undefined
  try {
    config.instanceId = await initializeStateIdentity(config.stateDir, config.instanceId)
    const store = new TaskStore({
      stateDir: config.stateDir,
      encryptionKey: config.stateEncryptionKey || Buffer.alloc(32),
    })
    manager = new TaskManager(config, store, new StrictAppServerExecutor(config))
    ...
    const close = async (): Promise<void> => {
      ...
      closing = (async () => {
        ...
        try {
          ...
          await serverClosed.finally(() => stateLease.release())
        }
      })()
      return closing
    }
