...
export function buildCapabilityManifest(config: AppConfig, manager: TaskManager): Record<string, unknown> {
  ...
  return {
    ...
    features: {
      ...
      abort: true,
      images: true,
      image_generation: config.imageGenerationMode === 'local',
      image_generation_local_files: config.imageGenerationMode === 'local',
      image_generation_max_bytes: config.imageGenerationMaxBytes,
      output_schema: true,
