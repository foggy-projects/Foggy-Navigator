...
test('operations scripts use isolated paths, port 3062, hidden start, and shutdown grace budget', () => {
  ...
  const installPs = fs.readFileSync('install.ps1', 'utf8')
  const installSh = fs.readFileSync('install.sh', 'utf8')
  const installEnvWriter = fs.readFileSync('scripts/configure-install-env.mjs', 'utf8')
  ...
  assert.match(updatePs, /scripts\\configure-install-env\.mjs/)
  assert.match(updateSh, /scripts\/configure-install-env\.mjs/)
  assert.match(installPs, /\[IO\.DriveInfo\]::GetDrives\(\)/)
  assert.match(installPs, /\.IsReady/)
  assert.match(installPs, /\$Root -match '\^\[A-Za-z\]:\\\\\$'/)
  assert.doesNotMatch(installPs, /\.Equals\('C:\\'/)
  assert.match(installPs, /configure-install-env\.mjs/)
  assert.match(installSh, /configure-install-env\.mjs" "\$INSTALL_DIR\/\.env" '\/'/)
  assert.match(installEnvWriter, /dotenv\.parse/)
  assert.ok(installPs.indexOf('if (-not (Test-Path $EnvFile))') < installPs.indexOf('configure-install-env.mjs'))
  assert.ok(installSh.indexOf('if [[ ! -f "$INSTALL_DIR/.env" ]]') < installSh.indexOf('configure-install-env.mjs'))
