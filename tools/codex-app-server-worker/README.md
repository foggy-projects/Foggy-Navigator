...
- Normal coding turns force Codex `features.image_generation=false` for both `thread/start` and
  `thread/resume`. If app-server emits an image item anyway, the Worker fails the turn as
  `APP_SERVER_UNEXPECTED_IMAGE_GENERATION`, interrupts the exact turn, suppresses the image/base64
  event, and retires that process. This remains the default until a pinned Codex CLI upgrade passes
...
6. Keep `CODEX_APP_SERVER_IMAGE_GENERATION_MODE=disabled` for coding sessions. Use `local` only for
...
7. Run `npm install`, `npm test`, `npm run typecheck`, and `npm run build`.
8. Run `npm run verify:schema` whenever the pinned CLI or app-server protocol changes.
