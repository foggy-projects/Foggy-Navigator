...
#
# This path is deliberately fail-closed: it never uses a forceful process
# termination and never treats a port alone as Worker ownership proof.
...
$configuredPort = Get-DotEnvValue -Path $EnvFile -Key "CODEX_WORKER_PORT"
if ($configuredPort) {
    [int]$parsedPort = 0
    if (-not [int]::TryParse($configuredPort, [ref]$parsedPort) -or $parsedPort -lt 1 -or $parsedPort -gt 65535) {
        Write-Host "Invalid CODEX_WORKER_PORT: $configuredPort" -ForegroundColor Red
        exit 2
    }
    $Port = $parsedPort
}
...
function Test-OwnedCodexWorker {
    param([int]$ProcessId)
    ...
    $hasWorkspacePath = $commandLine.IndexOf($ScriptDir, [System.StringComparison]::OrdinalIgnoreCase) -ge 0 -or
        $executablePath.IndexOf($ScriptDir, [System.StringComparison]::OrdinalIgnoreCase) -ge 0
    $hasEntrypoint = $commandLine -match "(?i)src[\\/]index\.ts"
    $hasNodeRuntime = $commandLine -match "(?i)(node|tsx)"

    return $hasWorkspacePath -and $hasEntrypoint -and $hasNodeRuntime
}
...
function Request-NonForcedStop {
    foreach ($processId in $ListenerProcessIds) {
        if (Test-ProcessExists -ProcessId $processId) {
            Write-Host "Requesting non-forced Codex Worker stop (PID=$processId)..." -ForegroundColor Yellow
            try {
...
if ($ListenerProcessIds.Count -eq 0) {
    ...
    Write-Host "No Codex Worker listener found on port $Port." -ForegroundColor Gray
    exit 0
}

foreach ($processId in $ListenerProcessIds) {
    if (-not (Test-OwnedCodexWorker -ProcessId $processId)) {
...
$workerToken = Get-DotEnvValue -Path $EnvFile -Key "CODEX_WORKER_TOKEN"
...
if ($null -eq $snapshot) {
    ...
    Write-Host "Refusing to stop: unable to prove the Codex Worker task snapshot is quiescent." -ForegroundColor Red
    exit 2
}
...
if ($SnapshotActive -ne 0 -or $SnapshotTotal -ne 0) {
    # PowerShell has no portable, non-destructive equivalent of SIGTERM for a
    # detached Node process. Retain this Worker for an explicit drain instead.
    Write-StopEvidence -Result "WORKER_DRAIN_UNCONFIRMED" -Action "none" -Detail "preflight_not_quiescent_no_windows_drain_signal"
    Write-Host "Codex Worker has active or unverified managed work; automatic restart is blocked. See $EvidenceFile" -ForegroundColor Red
    exit 2
}
...
if (-not (Wait-ForExit)) {
    ...
    Write-Host "Codex Worker did not exit within 30 seconds; no forced termination was attempted." -ForegroundColor Red
    exit 2
}
...
Write-Host "Codex Worker stopped after a quiescent snapshot." -ForegroundColor Green
