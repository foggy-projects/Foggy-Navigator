#!/bin/bash
# Codex Agent Worker - Installer (Linux / macOS)
...
if [ "$1" = "--upgrade" ] || [ -f "$INSTALL_DIR/VERSION" ]; then
    ...
    echo -e "${CYAN}Upgrading Codex Agent Worker: ${OLD_VERSION} -> ${VERSION}${NC}"
else
    echo -e "${CYAN}Installing Codex Agent Worker v${VERSION}${NC}"
fi
...
if ! command -v codex >/dev/null 2>&1; then
    echo -e "${YELLOW}WARNING: Codex CLI not found in PATH.${NC}"
    echo -e "${YELLOW}The worker can still start, but subscription login mode requires 'codex'.${NC}"
    echo ""
fi
...
chmod +x "$INSTALL_DIR/bin/codex-worker"
if [ -f "$SCRIPT_DIR/bin/codex-worker.ps1" ]; then
    cp "$SCRIPT_DIR/bin/codex-worker.ps1" "$INSTALL_DIR/bin/"
fi
...
if [ -n "$CODEX_WORKER_URL" ]; then
    ...
    if grep -q "^CODEX_WORKER_URL=" "$ENV_FILE" 2>/dev/null; then
        sed -i.bak "s|^CODEX_WORKER_URL=.*|CODEX_WORKER_URL=$CODEX_WORKER_URL|" "$ENV_FILE"
        rm -f "${ENV_FILE}.bak"
    else
        {
            echo ""
            echo "# Auto-upgrade URL (set by remote installer)"
            echo "CODEX_WORKER_URL=$CODEX_WORKER_URL"
        } >> "$ENV_FILE"
    fi
...
ACTIVE_CLI_PATH=$(install_cli_link "codex-worker" "$INSTALL_DIR/bin/codex-worker")
...
if ! echo ":$PATH:" | grep -q ":$ACTIVE_CLI_DIR:"; then
    if [ -n "$SHELL_RC" ] && ! grep -qF "$ACTIVE_CLI_DIR" "$SHELL_RC" 2>/dev/null; then
        echo -e "${YELLOW}To add 'codex-worker' to your PATH, run:${NC}"
...
if [ "$IS_UPGRADE" = true ]; then
    echo -e "${GREEN}Codex Agent Worker upgraded to v${VERSION}${NC}"
else
    echo -e "${GREEN}Codex Agent Worker v${VERSION} installed!${NC}"
fi
echo -e "${GREEN}============================================${NC}"
echo ""
echo -e "Commands:"
echo -e "  ${CYAN}codex-worker start${NC}     Start the worker"
echo -e "  ${CYAN}codex-worker stop${NC}      Stop the worker"
echo -e "  ${CYAN}codex-worker status${NC}    Check status & health"
echo -e "  ${CYAN}codex-worker version${NC}   Show version"
echo -e "  ${CYAN}codex-worker logs${NC}      Tail log output"
echo -e "  ${CYAN}codex-worker upgrade${NC}   Upgrade to new version"
echo -e "  ${CYAN}codex-worker upgrade-sdk${NC}   Upgrade Codex SDK only"
