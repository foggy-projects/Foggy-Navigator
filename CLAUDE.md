...
12. **异常与事务治理规范**：后端可预期业务异常、资源 not-ready、readiness/preflight blocker 不得被裸抛成 HTTP 500。Service 层需要区分系统缺陷与业务/资源状态异常；会被外层捕获并转换为 structured response 的事务方法，必须配置合适的 `noRollbackFor`，避免 `UnexpectedRollbackException` 覆盖真实响应。详细规范见 [后端异常处理与事务治理规范](docs/dev-specs/exception-handling-and-transaction-governance.md)。
