package com.foggy.navigator.coding.agent.api.config;

import org.springframework.boot.autoconfigure.AutoConfiguration;
import org.springframework.boot.autoconfigure.domain.EntityScan;
import org.springframework.context.annotation.ComponentScan;
import org.springframework.data.jpa.repository.config.EnableJpaRepositories;

/**
 * Coding Agent 模块自动配置
 * 提供 OpenHands 集成、会话管理、容器管理等功能
 */
@AutoConfiguration
@ComponentScan(basePackages = {
        "com.foggy.navigator.coding.agent.api.controller",
        "com.foggy.navigator.coding.agent.api.service",
        "com.foggy.navigator.coding.agent.api.repository",
        "com.foggy.navigator.coding.agent.api.listener",
        "com.foggy.navigator.coding.agent.api.sse",
        "com.foggy.navigator.foundation",
        "com.foggy.navigator.coding.agent.api.model"
})
@EntityScan(basePackages = {
        "com.foggy.navigator.coding.agent.api.model.entity"
})
@EnableJpaRepositories(basePackages = {
        "com.foggy.navigator.coding.agent.api.repository"
})
public class CodingAgentAutoConfiguration {
}
