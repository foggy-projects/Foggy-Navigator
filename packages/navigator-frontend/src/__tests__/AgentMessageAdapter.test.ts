...
describe('agentMessageAdapter', () => {
  ...
  it.each([
    ...
    const result = agentMessageAdapter.convert(raw, 'session-1')
  ...
  it('passes TOOL_CALL_START payload through unchanged', () => {
    ...
    const result = agentMessageAdapter.convert(makeRaw('TOOL_CALL_START', payload), 's1')
  ...
  it('passes TASK_COMPLETED payload through unchanged', () => {
    ...
    const result = agentMessageAdapter.convert(makeRaw('TASK_COMPLETED', payload), 's1')
  ...
  it('uses provided sessionId, not the one in raw', () => {
    ...
    const result = agentMessageAdapter.convert(raw, 'override-session')
