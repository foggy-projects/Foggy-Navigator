...
#   .\scripts\start-build-frontend.ps1 -Force       # clean workspace dist & rebuild all
#   .\scripts\start-build-frontend.ps1 -SkipBuild   # skip build, only restart nginx
#   .\scripts\start-build-frontend.ps1 -BuildOnly   # build only, don't restart nginx
...
$FrontendDir = Join-Path $RepoRoot "packages\navigator-frontend"
...
$ModulesMeta = Join-Path $RepoRoot "node_modules/.modules.yaml"
...
function Test-PnpmInstallRequired {
    $requiredPaths = @(
        ...
        (Join-Path $RepoRoot "packages\foggy-chat\node_modules\@foggy\chat-core"),
...
if ($RunningContainers -match "^${ContainerName}$") {
    ...
    Write-Host ""
    Write-ColorText "  Rebuild:    .\scripts\start-build-frontend.ps1" "Gray"
    Write-ColorText "  Nginx only: .\scripts\start-build-frontend.ps1 -SkipBuild" "Gray"
