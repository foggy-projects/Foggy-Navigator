...
$JAR_PATH = Join-Path $RepoRoot "launcher\target\launcher-1.0.0-SNAPSHOT.jar"
$ENV_FILE = Join-Path $RepoRoot "launcher\.env"
$BACKEND_LOG = Join-Path $LOG_DIR "backend.log"
$BACKEND_ERROR_LOG = Join-Path $LOG_DIR "backend-error.log"
$HEAP_DUMP_PATH = Join-Path $LOG_DIR "heap-dump.hprof"
...
if ($started) {
    ...
    Write-Host "  - Output: $BACKEND_LOG" -ForegroundColor Gray
    Write-Host "  - Errors: $BACKEND_ERROR_LOG" -ForegroundColor Gray
    Write-Host ""
    ...
    Write-Host "  $BACKEND_LOG" -ForegroundColor Gray
    Write-Host "  $BACKEND_ERROR_LOG" -ForegroundColor Gray
    Write-Host ""

    if (Test-Path $BACKEND_ERROR_LOG) {
        Write-Host "Last 20 lines of error log:" -ForegroundColor Yellow
        Get-Content $BACKEND_ERROR_LOG -Tail 20
    }
