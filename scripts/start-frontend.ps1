...
$FRONTEND_PATH = Join-Path $RepoRoot $FRONTEND_DIR
$NGINX_DIST = "dist\nginx"
$LOCKFILE = Join-Path $RepoRoot "pnpm-lock.yaml"
$MODULES_META = Join-Path $RepoRoot "node_modules/.modules.yaml"
...
if ($Build -or $BuildOnly) {
    ...
    $srcDir  = Join-Path $FRONTEND_PATH "dist"
    $destDir = Join-Path $RepoRoot $NGINX_DIST
