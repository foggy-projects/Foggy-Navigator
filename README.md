...
- **远程编程**：通过 OpenHands 和 Claude Code Worker 在多台主机上执行编程任务
...
Navigator Frontend (Vue 3)
   ...
   ├── addons/coding-agent (OpenHands集成)
   └── addons/claude-worker-agent (远程Claude Code)
```
...
├── addons/coding-agent/        # 编程 Agent (OpenHands)
...
| 容器 | Docker (OpenHands, Claude Code Worker) |
