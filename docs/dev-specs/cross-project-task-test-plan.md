...
## 测试场景 8：NAVIGATOR_TOKEN 注入 + 个人技能

验证任务派发时自动注入 `NAVIGATOR_TOKEN` 环境变量，以及 Claude Code 个人技能能通过该变量调用 Navigator API。
...
2. 在 Workers 页面创建一个新任务，prompt 为：
   ```
   运行命令 echo $NAVIGATOR_TOKEN | head -c 10，告诉我输出结果（只需要前 10 个字符）
   ```
3. 等待任务完成

**预期结果**：
- [ ] Worker 日志中可见 `navigator_api_key` 字段（非 null）
- [ ] Claude Code 输出中显示 JWT token 前缀（如 `eyJ` 开头）
...
**预期结果**：
- [ ] Claude Code 调用 `curl http://localhost:8112/api/v1/coding-agents -H "Authorization: Bearer $NAVIGATOR_TOKEN"`
...
curl -N -X POST http://localhost:3031/api/v1/query \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "运行命令 echo $NAVIGATOR_TOKEN 并告诉我结果",
    "cwd": "<project-path>",
...
- [ ] 证明 Worker 正确将 `navigator_api_key` 注入到 `NAVIGATOR_TOKEN` 环境变量
...
6. **NAVIGATOR_TOKEN**：该环境变量由后端自动生成（JWT token），在派发任务时注入。只要用户已登录，token 就会自动生成，无需手动配置。技能应检测变量是否存在（未通过 Navigator 派发时不存在）并给出友好提示。
