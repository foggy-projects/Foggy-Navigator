...
补充：child frame 现在按子 Agent 风格兼容自然语言最终消息。显式 `submit_frame_result` / `handoff_to_parent` 仍是结构化状态、refs、受控退出的首选路径；`submit_skill_result` 仅作为旧名兼容 alias 保留。但如果 child 没有 tool call 而直接输出自然语言，运行时会归一化为：

1. 文本明显在追问或请求补充信息：`WAITING_FOR_USER_INPUT`，child 进入 `AWAITING_USER`，下一条用户消息继续直达该 child。
2. 普通最终答复：`FINAL_FOR_USER`，child 完成并将摘要作为 promoted result 提升给 parent。
...
- 2026-05-22: child frame 支持子 Agent 风格自然语言最终消息；显式 submit/handoff 仍是结构化首选，plain assistant 会归一化为完成或 `AWAITING_USER`。
