...
| --- | --- | --- | --- |
| `Worker` / WorkerPool | `PLATFORM` 或 `UPSTREAM_SYSTEM` | `UpstreamClientApp` / `Agent` | Worker 是执行环境，不应直接归属 UpstreamUser。即使由上游 admin 创建，也应通过 WorkerPool、capability、label 或 explicit grant 绑定给 App / Agent |
| `LlmConfigModel` | `PLATFORM` / `UPSTREAM_SYSTEM` / `CLIENT_APP` | `UpstreamClientApp` / `Agent` / 可选 `UpstreamUser` policy | Admin 创建的模型可作为上游系统共享模型；ClientApp 创建的模型只对该 ClientApp 及其用户可见 |
...
4. 创建上游系统级共享 Worker / WorkerPool / LLMConfigModel / WorkingDirectory。
...
3. 由 BizWorker / Navigator 解析可见资源，不接受客户端直接指定任意 worker/model/path。
...
10. Runtime submission 日志应能记录 resolved model / worker / workspace 的脱敏元数据，便于上游排查“为什么本次调用用了这个资源”。

## 11. 后续实现建议

1. 为 Admin credential 引入稳定的 `UpstreamSystemPrincipal` 显式实体或等价字段，停止把 key 当 owner。
2. 为 Worker / LLMConfigModel / WorkingDirectory / Agent 统一补齐 `ownerType`、`ownerId`、`createdByCredentialId`、`grantPolicy`、`enabled`、`disabledReason`。
...
5. 为 SDK / CLI 增加可观测命令：查看某个 ClientApp / upstreamUser / Agent 最终可用的模型、Worker、目录和授权来源。
