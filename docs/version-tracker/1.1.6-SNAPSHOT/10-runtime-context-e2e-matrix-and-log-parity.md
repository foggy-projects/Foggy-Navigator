...
| --- | --- | --- | --- |
| 普通多轮 Root turn | `test_scripted_llm_submission_log_matches_root_recent_conversation` | 无工具调用时第二轮为 `system -> human -> ai -> human`，历史 user / assistant 以独立 role messages 注入 | 第二轮 root event 初始 role 同为 `system -> user -> assistant -> user`，并记录 `persistent_turn_completed` |
...
| Child plain assistant subagent result | `test_llm_agent_child_plain_assistant_answer_completes_as_subagent_result`、`test_llm_agent_child_plain_assistant_clarification_pauses_for_user`、`test_llm_agent_invoke_business_skill_plain_clarification_bubbles_waiting_user_focus` | child 可无 tool call 直接返回自然语言；追问类文本进入 `AWAITING_USER`，普通最终文本进入 `COMPLETED` | implicit result event 使用 `tool_name=assistant_message`；parent active focus 仍指向 awaiting child |
...
| Root tool protocol retention | 待补：`test_scripted_root_tool_protocol_is_retained_next_turn` | 第 N 轮 Root 调用 `list_skill_resources` 或 business function 后，第 N+1 轮 LLM body 包含上一轮 assistant tool_call、匹配 tool_result、assistant final 和当前用户消息 | root event JSONL 可对账上一轮 tool_call / tool_result；child-private events 不出现在 Root body |
| Prompt pre-compaction warning | `test_scripted_root_prompt_budget_warning_is_logged` | 多轮 Root 回合在 `maxPromptMessages` 触顶前先压缩；第 5 轮 `llm-submissions.meta.runtimeWarnings` 包含 `PROMPT_BUDGET_PRE_COMPACTION`，body 中包含 compacted summary | 同一 task/frame 的 `runtime-message-events` 写入 `eventType=runtime_warning`，并继续记录 `persistent_turn_completed` |
...
普通下一轮应看到 Root-visible protocol。无工具调用时：
...
system
历史 user
历史 assistant tool_call
历史 tool_result
历史 assistant final
当前 user
```

不应重放 child-private raw tool call / tool result。child-private protocol 留在 execution evidence：
...
该信息只用于当前 parent 续跑。后续 Root 普通上下文只保留逐层 promoted 后的 Root-visible protocol，不写入 child-private raw trace。
