...
如果上一轮 task 进入终态 `FAILED`，但 Worker 报告的是可恢复 runtime 中断，例如 `LLM skill agent reached max iterations without valid submit`，上游仍应按续聊处理：创建新的 ask、传同一个 `contextId`、用 `继续` / `continue` 或用户补充说明作为新消息，并轮询新的 `taskId`。旧 `taskId` 只作为诊断证据，不作为继续执行入口。
...
`ACCOUNT_POLICY.md` / `AGENT.md` / `MEMORY.md` 是 Navigator 管理的账号上下文文件，不是模型侧隐藏记忆。存在时它们已经进入 Account Context system prompt；运行时文件工具可能仍能看到 delegated workspace 中的同名文件，但模型不应为了确认内容而再次读取，除非用户明确要求查看/维护这些文件或任务需要逐字文件正文。
