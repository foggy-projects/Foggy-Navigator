...
审批必须绑定 Client App、upstream user、Navigator effective subject、task/session、函数和 input hash。Client App 只能通过 Navigator Java 的确认或 callback API 更新审批状态，不能直接调用 Worker resume。
...
1. 审批通过只授权同一个 `client_app_id + upstream_user_id + task_id + session_id + script_run_id + suspend_id + function_id + input_hash`。
...
1. Java 生成确认码或确认挑战。
2. 前端或 Client App 向用户展示确认要求。
3. 用户提交确认码给 Java。
4. Java 校验确认码、审批人权限、Client App、upstream user 和任务绑定。
...
回调的授权主体是 `client_app_id`。上游多租户系统必须按 Navigator 已注册的 App 分别配置 callback credential，不能用一个上游系统级密钥横跨多个 App。
...
1. 校验回调归属的 `client_app_id`、签名或共享密钥。
2. 校验 approval 仍处于 `suspended` 且未过期。
3. 校验上游审批人和业务权限，且审批人属于当前 App。
4. 写入 approval result 和 audit。
5. 调用 Worker `resume_suspension`。

## Java Control Plane 到 Worker 的 Resume 触发

Java 通过独立的 Control Plane 端点接收管理员或 Client App 触发的 Resume 请求，Worker 网关自身不暴露 resume 控制面。Java 完成严格上下文和输入 Hash 校验后，通过内部事件机制（LangGraphWorkerResumeEventListener）分发到 Worker。
...
4. Java resume 必须绑定原始 input hash、`client_app_id` 和 `upstream_user_id`。
5. 审批摘要必须可审计，不能只保存自然语言。
6. 同一 `suspend_id` 只能完成一次最终状态转移。
7. 审批通过后仍需执行上游权限和业务规则校验。
8. 所有回调必须有签名、token 或可信网络边界校验。
9. Client App 的 runtime credential 不能用于创建新 App；创建 App 必须使用 provisioning credential。
