# Upstream App 身份、授权、Worker Pool 与 Skill 作用域契约
...
- purpose: 定义上游 App 与 Navigator Java 用户体系、授权凭证、Biz Worker Pool 和 Skill 作用域之间的边界
...
上游 App 不直接认识、选择或绑定具体 Biz Worker。Biz Worker 也不直接持有上游 App 的 runtime credential，不直接访问上游 REST，不自行判断上游用户权限。

Navigator Java 服务是唯一控制面，负责：

1. 注册和管理多个 Upstream App。
2. 注册和管理多个 Biz Worker 或 Biz Worker Pool。
3. 将上游请求路由到合适的 Biz Worker。
4. 管理鉴权、授权、Skill 作用域、函数授权、审批、审计、幂等和凭证注入。
5. 保存 Upstream App、upstream user、Navigator effective user、task/session、Worker 调度和 Business Function 调用之间的关联。
...
如果某个上游 SaaS 系统有 100 个租户，则在 Navigator 侧注册为 100 个 `upstream_app_id`。授权、Skill、函数白名单、审批策略、callback 配置和 Worker 路由策略都挂在 `upstream_app_id` 上。
...
| 主体 | 说明 | 所有者 | 是否可信 |
| --- | --- | --- | --- |
| `upstream_app_id` | Navigator 侧注册的上游接入单元；上游一个租户对应一个 App | Navigator Java | 是，来自 Java Registry |
| `upstream_user_id` | App 内部用户 ID，仅在 `upstream_app_id` 下唯一 | 上游 App | 需由 Java 校验签名后接受 |
| `upstream_user_display_name` | 上游用户展示名，可用于审批卡片和审计摘要 | 上游 App | 不作为权限依据 |
...
上游创建 App 前，需要先获得 Java 发放的接入授权码或 provisioning credential。该凭证只用于创建或初始化 Upstream App，不用于业务运行时调用。
...
App 创建成功后，Java 为该 App 颁发 runtime credential。它只代表该 `upstream_app_id`，不能创建新 App。
...
| `mapped_user` | `upstream_app_id + upstream_user_id` 映射到具体 Navigator 用户 | 需要内部用户级隔离时 |
| `fixed_owner` | 类似 Sharing Key，所有调用映射为 App owner | 仅兼容低风险试点，不推荐承载正式业务 |

推荐首版默认使用 `service_account_with_actor`。原因是上游 App 的授权边界应由 App Grant 决定，而不是由某个内部用户角色间接扩大。
...
1. 上游 App 授权的是业务能力，不是具体 Worker。
2. Worker 实例可以继续物理绑定 Navigator 用户，但 App 不直接依赖该用户。
3. Java 根据 App Grant 和路由策略选择 Pool，再选择 Worker。
4. task scoped token 必须绑定 `upstream_app_id`、`task_id`、`session_id`、`skill_id`、函数作用域和有效期。
...
2. `upstream_app_skill`：某个 Upstream App 自己维护的 App 作用域技能。
3. `builtin_public_skill`：Navigator Java / Biz Worker 侧提供的平台内置公共技能。

`role_skill` 本版本不考虑，不参与 1.1.3 的 Skill 暴露和授权计算。

### Upstream App Skill 规则

Upstream App 创建后，可以维护自己的 App 作用域 Skill。该 Skill：

1. 只在当前 `upstream_app_id` 作用域内可见。
...
其中 `authorized upstream_app_skill` 必须由 Java 授权，例如用户是 App owner、管理员、协作者或被显式授予调试/调用权限。角色技能不在本版本参与。
...
Worker 可接收 `upstream_app_id`、脱敏后的 `upstream_user` 摘要、`skill_id` 和 task scoped token 绑定信息，但不能接收 runtime credential、App secret、上游用户权限明细或 Worker 路由决策密钥。
...
1. Sharing Key 没有 `upstream_app_id`。
2. Sharing Key 没有上游用户身份映射。
3. Sharing Key 的 allowlist 是接口操作级，不是 Business Function scope。
4. Sharing Key 的上下文实际归属 `ownerUserId + agentId`，不适合多个上游 App 共享。
...
- development_progress: Upstream App 身份、授权、Worker Pool 与 Skill 作用域契约已拆出
...
5. `service_account_with_actor` 是否作为所有正式 Upstream App 的默认映射模式。
