...
- `@foggy/navigator-chat-widget` 已提供可选默认弹窗 shell：传入 `suspension`、`suspensionDialogVisible`、`suspensionSubmitting`，监听 `suspensionDecision`。
- `NavigatorChat` 也提供 `#suspension-dialog` slot，宿主可以替换默认弹窗。
- 默认弹窗仍只发出决策事件，必须通过上游 BFF 转发 resume。
- UI + BFF 最小示例见：[16-upstream-suspension-ui-bff-demo.md](./16-upstream-suspension-ui-bff-demo.md)。

## NavigatorChat 默认弹窗

```vue
<NavigatorChat
  v-model:suspension-dialog-visible="visible"
...
```vue
<NavigatorChat :config="chatConfig" :suspension="suspension">
  <template #suspension-dialog="{ suspension: item, visible, updateVisible, submit }">
    <YourDialog
      :model-value="visible"
      :item="item"
      @update:model-value="updateVisible"
      @approve="submit({ suspendId: item.suspendId, suspensionType: item.suspensionType, decision: 'approved' })"
    />
  </template>
</NavigatorChat>
```
