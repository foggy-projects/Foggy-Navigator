...
`response.content` 默认为空字符串，tool-only response 可省略。`tool_calls` 支持两种输入格式：

1. LangChain 风格：`{"name":"invoke_business_skill","args":{...}}`。
2. OpenAI Chat Completions 风格：`{"function":{"name":"invoke_business_skill","arguments":{...}}}`。

mock LLM 输出给 OpenAI client 时会统一归一化为 `function.name/function.arguments`，其中 `arguments` 为 JSON string；流式响应必须以 `finish_reason=tool_calls` 收尾，并继续发送 `data: [DONE]`。
...
| Java-side upstream BizWorker E2E | pass | `mvn -pl addons/langgraph-biz-worker -am -Dtest=BusinessAgentLanggraphLaunchE2ETest "-Dsurefire.failIfNoSpecifiedTests=false" -DfailIfNoTests=false test`；`mvn -pl addons/langgraph-biz-worker -am test`，覆盖 Business Agent task -> LangGraph task -> `WorkerTaskStartEvent` runtimeContext |
