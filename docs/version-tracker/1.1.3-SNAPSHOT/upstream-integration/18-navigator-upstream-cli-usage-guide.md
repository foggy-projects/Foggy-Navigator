...
上游 Agent 应在 live actor smoke 前优先执行 `upstream verify-agent-readiness`，确认 agent 注册、ClientApp skill grant、upstream user grant 与 model config grant 均可用；需要确认真实 runtime 会解析到哪个 Agent、Model、WorkerPool 和 Workspace 时，执行 `upstream inspect runtime`；需要查看已授权 skill 交付内容时，再通过 `upstream skill tree` 和 `upstream skill read` 读取文件树与指定文本切片。能力设计见 [20-navigator-upstream-cli-readiness-and-skill-artifact.md](./20-navigator-upstream-cli-readiness-and-skill-artifact.md)。
...
```text
install into upstream project -> runtime-token --write-profile -> inspect runtime -> verify-agent-readiness -> ensure-grant -> ask -> messages --poll
```
