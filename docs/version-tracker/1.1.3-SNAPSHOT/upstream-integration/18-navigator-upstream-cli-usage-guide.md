...
`client-app issue-runtime-key` / `issue-runtime-credential` 需要 `navigator-upstream-cli` / `navigator-open-sdk` `1.0.6` 或更高版本。`modelVariant`、公共安装包覆盖升级清理、以及更严格的 owner-aware smoke 建议使用 `1.0.7` 或更高版本。
...
### Agent / Model / Workspace 标准接入口径

上游运行时推荐只面向 Agent 编程，把 Agent 当成稳定 runtime profile：

1. Agent 绑定默认 `LlmConfigModel`、默认 `WorkingDirectory`、可用工具 / Skill / function。
2. `LlmConfigModel.workerBackend` 决定使用 Claude Code、OpenAI Codex、Gemini CLI 或 LangGraph Biz 等 backend。
3. `modelVariant` 只在同一个 `LlmConfigModel` 内选择具体模型名，例如 `sonnet` / `opus` / `qwen3.5-plus`；它不改变 backend、credential、workspace 或 physical worker。
4. 新任务可在 Agent policy 允许范围内传 `--model-config-id` / `--model-variant` 覆盖默认值；同一 task / context 后续续聊不应再改变已经冻结的 effective config。
5. `WorkerPool` 是内部 routing artifact。上游 bootstrap、文档、SDK 使用说明不应要求业务调用方理解或传入 WorkerPool；短期 CLI 中出现 `worker-pool-id` 时，只作为旧绑定接口和诊断信息。

一个 clean ClientApp 在发起真实 ask 前至少应通过：

```powershell
.\tools\navigator-upstream\navi.ps1 upstream model grant --profile <tenantProfile> --model-config-id <modelConfigId> --set-default --write-profile
.\tools\navigator-upstream\navi.ps1 upstream directory client-init --profile <tenantProfile> --file .\.navigator\client-directory.json --write-profile
.\tools\navigator-upstream\navi.ps1 upstream agent sync --profile <tenantProfile> --manifest .\.navigator\agent.json
.\tools\navigator-upstream\navi.ps1 upstream agent set-default-model --profile <tenantProfile> --agent-code <agentCode> --model-config-id <modelConfigId>
.\tools\navigator-upstream\navi.ps1 upstream agent set-default-workspace --profile <tenantProfile> --agent-code <agentCode> --directory-id <directoryId>
.\tools\navigator-upstream\navi.ps1 upstream ensure-grant --profile <tenantProfile> --upstream-user-id <upstreamUserId>
.\tools\navigator-upstream\navi.ps1 upstream owner-smoke --profile <tenantProfile> --upstream-user-id <upstreamUserId> --agent-code <agentCode>
```

`directory client-init` 的 manifest 必须显式包含 `workspaceScope`：
...
如果 `owner-smoke` 输出 `internalRoute.workerPoolId` 但缺少 `effectiveWorkerBackend` / `effectivePhysicalWorkerId`，说明当前 Agent 或模型仍停留在旧 WorkerPool-only 路径，不能作为新 owner-aware runtime 的合格入口。应修复 `LlmConfigModel.workerBackend`、PhysicalWorker backend capability、workspace worker 归属和 Agent binding，再进入 ask/messages。
...
### Upstream Admin WorkerPool 兼容闭环

`NAVI_ADMIN_API_KEY` 可以管理该上游系统主体下的 BizWorker identity 和 WorkerPool。该能力是现阶段内部 routing 兼容面，不是上游标准接入面；新接入应优先通过 Agent + `LlmConfigModel.workerBackend` + `WorkingDirectory` 解析物理执行路线。短期仍需要准备 WorkerPool 时，推荐顺序：
...
4. `skill sync --scope client-app-public` 和账号私有 skill sync 没有 `agentId` 参数。它们走 ClientApp control-plane 线路，服务端根据当前 `tenantId + clientAppId` 解析可见 Biz Worker 目标并 materialize；优先使用当前 ClientApp / UpstreamSystem 可见的 Agent worker route，WorkerPool route 会 fan-out 到启用且健康的 Biz worker 成员，没有 Agent route 时使用当前 UpstreamSystem 下启用且健康的 Biz worker identity。`NAVI_ADMIN_API_KEY` / `ADMIN_KEY` 只用于上游管理权限，不用于选择 materialize 目标。
5. 上游不要在 skill sync 或 live ask metadata 里补 `skill_name`、`skillId`、`businessSkillName`、`businessSkillId` 这类隐藏路由字段。验证指定业务 skill 时，把目标 skill 写进用户消息，例如“请使用 <skillId> 技能，完成 <task>”。
6. 上游交付 smoke 至少验证 `upstream function --help`、`function import`、`function grant` 和 `function visible` 可用。
