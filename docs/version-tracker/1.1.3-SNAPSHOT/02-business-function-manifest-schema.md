...
Business Function Manifest 是上游 REST API 进入 Navigator 的受控声明。它不是给 LLM 执行的 HTTP 描述，也不是 Worker 的业务实现代码。Manifest 的核心作用是让 Java 服务可以统一管理业务函数的版本、权限、审批、幂等、审计、适配和 Skill 暴露边界。
...
| `auth_scope` | 是 | `java_only` | Java 权限校验和上游授权域，不直接暴露给 LLM |
...
| `audit` | 是 | `audit_runtime` | 审计业务键、脱敏字段、操作者和租户绑定规则 |
| `exposure` | 是 | `registry_control` | 控制是否可被 root planner、script engine、Skill 使用 |
| `skill_allowlist` | 是 | `registry_control` | 允许哪些 Skill 使用该函数，默认空列表等价于不可见 |
...
idempotency:
  required: true
  key_template: close_apply_submit:${tenant_id}:${application_id}
...
审计记录必须能回答：谁、在哪个租户、通过哪个任务、哪个 Skill、哪个函数版本、用什么业务键、经过什么审批、调用了哪个上游结果。
