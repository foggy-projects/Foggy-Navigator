...
- [x] action 明确携带 `label/pageLabel/pageName/routeLabel` 时生成业务按钮文案；未携带时不根据 `routeName` 推断业务文案。
...
- status: completed
- summary: `extractActions` 已扩展候选源与递归容器；`collectActions` 增加 WeakSet 防循环；dedupe 使用稳定序列化 payload。业务 action 展示改为 pending 后合并到最终回复，按钮文案使用 action 显式 `label/pageLabel/pageName/routeLabel`，终态时补齐 skill frame 收口。
...
- resolution: 工具消息中的业务 action 不再立即渲染，改为暂存并合并到最终业务回复；去重同一 action；按钮文案由上游在 `structured_output.label` 或 `pageLabel/pageName/routeLabel` 中明确提供，Widget 不根据 `routeName` 推断业务中文名；任务进入终态时将仍 running 的 skill frame 兜底置为完成或失败。

### Upstream Action Guidance

- 上游需要为面向用户的按钮文案负责；Widget 不维护业务路由名到中文页面名的映射。
- 上游 skill 返回 `structured_output` 时应显式携带 `label`，或携带 `pageLabel/pageName/routeLabel`。
- `routeName` 仍用于宿主路由跳转，但不作为用户可见文案的来源。
...
- [x] 重复 action 只展示一次，且显式 `label` 会作为按钮文案；未携带业务文案时不从 `routeName` 猜测中文。
