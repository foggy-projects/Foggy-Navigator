...
上游业务系统不直接调用 LangGraph Biz Worker，Worker 也不作为业务实现方。
...
1. 上游系统边界：只需要理解 Navigator Java 暴露的业务 Agent API、回调协议和能力注册协作要求。
...
- `risk_level` 和 `approval_policy` 决定调用前是否必须暂停并等待用户确认。
...
- `list_business_functions(domain?, intent?)`：查询当前任务、账号、Skill 可见的业务函数摘要。
...
```json
{
  ...
  "user_id": "user-a",
  "source_system": "tms",
  "idempotency_key": "close_apply_submit:app-001"
}
```

Java 侧必须基于该上下文做权限校验、函数可见性校验、审计落库和上游凭证注入。
...
| Navigator Java 服务 | 承载 Function Registry、REST Adapter、凭证、权限、审批、审计、幂等、错误归一化 |
| LangGraph Biz Worker | 执行自然语言推理、Skill 路由、脚本编排、标准工具调用与暂停恢复 |
