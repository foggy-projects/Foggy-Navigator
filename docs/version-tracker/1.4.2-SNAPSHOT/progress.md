...
| [BUG-006](./workitems/BUG-006-diagnostic-actions-interaction.md) | ready-for-verification | 错误详情/复制/分享动作已阻止冒泡并显式为 button；错误卡片复用 HTTP 复制降级 helper | passed-local：`foggy-chat` 6 个测试文件共 114 tests passed；chat package build 与 Navigator frontend `build:check` passed | ready-for-PC-verification：前端静态产物已更新，等待 PC 端验证详情不再收起及 HTTP 复制成功 | 本地 HTTP 响应 `Last-Modified` 与更新后的 `navigator-frontend/dist/index.html` 一致；构建运行于 Node 24，和仓库 Node 22 基线不同，保留 engine warning |
