...
| P8 | 结构化错误诊断、诊断快照、登录态详情和临时分享链接 | implementation-complete / verification-partial | Provider 无关错误信封、Codex 两类 Worker、90 天快照、owner/tenant 隔离、7/30 天 hash-token 分享、匿名安全页面、Task/SSE/chat UI 已落地；分享默认关闭。结构化诊断 SDK Worker `1.0.16` 已发布，目标运行时仍待升级。Java 19 tests、Worker 全量测试/typecheck、前端 typecheck/chatState 测试通过；Node 22 组件/build、migration、浏览器和正式门禁待补，见 `EXEC-142-021`、`BUG-005` |
...
| [BUG-005](./workitems/BUG-005-codex-worker-structured-error-release.md) | ready-for-verification | Worker `1.0.16` 已发布，包含已实现的结构化脱敏错误事件协议 | passed-release-candidate：177 tests 中 176 passed/1 Windows-only skipped；typecheck、build、三平台包与 full smoke 通过；发布器回读校验全部产物 | blocked：目标 Worker 仍为 `1.0.14`，尚未取得其 SSH 升级权限，不能完成 Terra 失败详情的运行态复测 | 发布提交 `e02fffb2`；公开 `latest.json` 已确认为 `1.0.16`。升级后须验证 `/health` 与诊断详情非空后才能关闭 |
| [BUG-006](./workitems/BUG-006-diagnostic-actions-interaction.md) | ready-for-verification | 错误详情临时改为 Teleport 弹窗；错误详情/复制/分享动作已阻止冒泡并显式为 button；错误卡片复用 HTTP 复制降级 helper。根因为虚拟列表未跟踪异步内联详情的高度变化，不是延时关闭 | passed-local：`foggy-chat` 6 个测试文件共 114 tests passed；chat package build 与 Navigator frontend `build:check` passed | ready-for-PC-verification：前端静态产物已更新，等待 PC 端验证详情弹窗稳定保留及 HTTP 复制成功 | 本地 HTTP 响应 `Last-Modified` 与更新后的 `navigator-frontend/dist/index.html` 一致；构建运行于 Node 24，和仓库 Node 22 基线不同，保留 engine warning。恢复内联展示前须补虚拟项尺寸依赖或显式重新测量 |
