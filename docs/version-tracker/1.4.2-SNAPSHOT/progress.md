...
| [BUG-006](./workitems/BUG-006-diagnostic-actions-interaction.md) | in-progress / permanent-fix-pending | 临时弹窗、按钮冒泡隔离和 HTTP 复制降级已部署；根因为虚拟列表未跟踪异步内联详情的高度变化，不是延时关闭 | passed-local：`foggy-chat` 6 个测试文件共 114 tests passed；chat package build 与 Navigator frontend `build:check` passed | ready-for-PC-verification：前端静态产物已更新，等待验证弹窗稳定保留及 HTTP 复制成功 | 永久内联修复按用户决定登记为待排期：恢复内联前必须补虚拟项尺寸依赖或精确重新测量，并覆盖滚动回收回归。构建运行于 Node 24，和仓库 Node 22 基线不同，保留 engine warning |
