...
| `addons/codex-worker-agent` | Codex Provider、任务与流式能力，并检查 Codex SDK Worker 连接 | 已消费 Worker `ready`：显式 `false` 阻断连接，缺字段兼容旧 Worker；继续统一外部任务约束、渐进拆分 `CodexTaskService`，仓内迁移后直接删除旧契约 | 可依赖 session/business/framework；平台消费 Worker 健康契约，不反向决定 Worker execution policy | 把缺少 `ready` 的兼容行为当作 external-ready；在仓内引用未处理时删除旧契约；误删当前 Provider 能力 | [GOV-001](./workitems/GOV-001-internal-external-trust-boundary.md)、[CLEAN-004](./workitems/CLEAN-004-experimental-and-legacy-addon-governance.md)、[OPT-002](./workitems/OPT-002-core-code-maintainability.md) |
| `addons/langgraph-biz-worker` | LangGraph Biz Provider、Worker 健康/工具授权与旧审批链路 | 已接收 Worker 外部状态并将显式 `ready=false` 映射为 `OFFLINE`，缺字段兼容旧 Worker；继续迁移 taskId/reviewedBy 链路并直接删除旧 LangGraph API | 可依赖 session/business/framework；平台只消费 Worker readiness，不替 Worker 声明策略完成 | 继续以请求体 reviewer 作为审计主体；把旧 Worker 兼容路径当 external-ready；删除新受控审批能力 | [GOV-001](./workitems/GOV-001-internal-external-trust-boundary.md)、[GOV-002](./workitems/GOV-002-biz-worker-and-upstream-user-boundary.md)、[CLEAN-004](./workitems/CLEAN-004-experimental-and-legacy-addon-governance.md) |
...
| `metadata-query-module` | 旧 Foggy Dataset/FSScript 查询链路；模块、根 reactor 条目、launcher 依赖/专属 bean 断言、专属 Skill 与当前文档已移除 | 保持切片退出；启动/浏览器 smoke 和 P7 正式门禁后续补证 | 根 reactor 当前为 16 个模块；删除后 metadata-config/launcher clean test 15/15 `SUCCESS`，依赖树与 clean target 无旧查询依赖 | 把本地 clean 结果冒充 hosted CI、启动体验或正式验收；误删 `metadata-config-module`；未确认目标环境时执行数据操作 | [CLEAN-003](./workitems/CLEAN-003-metadata-query-retirement-audit.md) |
...
当前实现只完成 GOV-001/GOV-002 的首批路由与 readiness 门禁，状态仍为 `in-progress`。没有完成 task token、可信 upstream identity、审批/恢复/取消绑定、审计可靠性或生产/外部启用，不得据此进入正式签收。
