...
| `metadata-config-module` | 元数据配置能力；23 个 tracked files 保留、本批业务树 diff 为 0 | 保留；删除后定向 clean test 已通过，继续维护边界标注 | 由明确消费者依赖 | 因名称或 Java package 前缀相似而随 metadata-query 误删 | [CLEAN-003](./workitems/CLEAN-003-metadata-query-retirement-audit.md) |
