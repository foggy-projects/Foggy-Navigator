...
- external_enablement: no
- production_enablement: no
- current_scenario: P1A foundation/shadow、P1B-A fixture-only typed-management authentication core、P1B-B0 pure-offline pre-seed inventory validator 与 P1C-A CLI/SKILL permission visibility 均已独立 accepted。P1C-A 不 seed、签发或验证真实 SIM/TMS 主体、凭据、mapping 或 owner approval，且不构成 external 或 production readiness。开发期 SIM/TMS 联调 MVP 已通过独立签核；它只验收代码、CLI、手册和本地 preflight，仍未验证真实上游 runtime。BUG-007 已修复 task capability schema contract 并完成本机 MySQL validate；runtime retry 因 TMS private profile 非 `0600` 被 fail-closed 阻断，未发起 runtime task。INT-001 正常 disposable runtime 链路通过，但独立 forced-SIGNAL cleanup 以 `FAILED_CLEANUP/SIGNAL` 失败，故 INT-001 被拒绝；BUG-008 server-derived owner-context 修复以 `accepted-with-risks` 签收。BUG-009 已完成 held-child topology、schema-v4 receipt 和 parent-adoption duplicate-key fail-closed 回归；`k6f8m2q9` 在 proof 前 fail closed，随后两个全新 runs 仅得到“无 root receipt、`private/` 存在、7 个 non-receipt root entries”的最小快照。它们不构成成功或可诊断证据；BUG-009 因此为 `NEEDS_REPLAN`。以上均不替代真实上游验收。
- next_action: [BUG-009](./workitems/BUG-009-int001-forced-signal-owned-cleanup.md) 的 [历史 runtime record](./test-records/BUG-009-int001-forced-signal-runtime-2026-07-21-k6f8m2q9.md) 与 [follow-up record](./test-records/BUG-009-int001-forced-signal-runtime-follow-up-2026-07-21.md) 已如实记录所有允许观察。不得读取、重试或手工清理任何 failed run，也不得把 `CLEANED/UNKNOWN` 或缺失 receipt 误作 forced-SIGNAL 成功。下一步需要单独批准固定枚举、root-level 的诊断/执行投影方案；它不得接触真实 TMS/SIM、shared `8112`、profile/credential、Worker、Gateway、Pool、identity、Codex route、external 或 production，也不得放宽 strict ownership。BUG-009 未具备独立签核资格，INT-001 仍为 rejected，不能开始真实上游 acceptance。P1B-B/P2/P3/P4、Gateway strict、Worker external 与 production 均保持 deferred。
...
| [BUG-009 INT-001 forced-SIGNAL owned cleanup](./workitems/BUG-009-int001-forced-signal-owned-cleanup.md) | acceptance-found harness lifecycle defect；健康 disposable stack 的 parent-TERM cleanup 仍返回 `FAILED_CLEANUP` | NEEDS_REPLAN；held-child topology、receipt v4 与 duplicate-key fail-closed regression 已完成，但两个新 run 无 root receipt，需独立批准 root-level diagnostic/projection 方案；见 [follow-up record](./test-records/BUG-009-int001-forced-signal-runtime-follow-up-2026-07-21.md) |
