...
- external_enablement: no
- production_enablement: no
- current_scenario: P1A foundation/shadow、P1B-A fixture-only typed-management authentication core、P1B-B0 pure-offline pre-seed inventory validator 与 P1C-A CLI/SKILL permission visibility 均已独立 accepted。开发期 SIM/TMS 联调 MVP 仍只证明代码、CLI、手册和本地 preflight。INT-001 仍因 forced-SIGNAL AC-2 被拒绝。BUG-009 fixed-enum projection 与 46 个离线回归已完成；唯一批准的新 run `p7m3c6r2` 成功提供 projection/stdout/root receipt，但 strict listener proof 为 `socket-listener-absent`、TERM 为 `0`，held child 最终 `CLEANED/UNKNOWN + HOLD_TIMEOUT`。因此 BUG-009 仍为 `NEEDS_REPLAN`，以上均不替代真实上游验收。
- next_action: [最新 runtime record](./test-records/BUG-009-int001-fixed-enum-projection-runtime-2026-07-21.md) 已记录全部允许证据。不得重试该 runId、启动另一 rehearsal、读取历史/当前 failed run 的 `private/children/log/profile/payload/process/Docker`，也不得把 `CLEANED/UNKNOWN` 解释为 forced-SIGNAL 成功。下一步需单独批准 strict supervisor listener proof 与 held-child `HEALTH_READY` 证据不一致的静态诊断/修复方案；不得放宽 owner/listener proof 或触及真实 TMS/SIM、shared `8112`、credential、Worker、Gateway、Pool、identity、Codex route、external 或 production。BUG-009 不具备独立签核资格，INT-001 仍为 rejected。
...
| [BUG-009 INT-001 forced-SIGNAL owned cleanup](./workitems/BUG-009-int001-forced-signal-owned-cleanup.md) | acceptance-found harness lifecycle defect；健康 disposable stack 的 parent-TERM cleanup 仍返回 `FAILED_CLEANUP` | NEEDS_REPLAN；fixed-enum projection 与离线 gate 已完成，但唯一批准 rehearsal 的 strict listener proof 缺失、TERM=0；不得重试，需新方案 |
