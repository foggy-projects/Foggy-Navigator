...
3. 接收或桥接 suspension 信息。
4. 转发 `approval_required` / `approval_result` / `script_resumed` 等事件。
5. 保存可复用脚本、脚本 hash、依赖 capability 和风险等级。
6. 将脚本结果纳入 Skill Frame 和最终任务输出。
...
5. Worker 将 suspension 映射为 `approval_required` 事件。
...
2. 上游提供一个 `orderBiz` 对象门面。
3. 对象门面包含：
   - `get_order`：只读
   - `close_apply_draft`：生成草稿
   - `close_apply_submit`：内部 pause，等待确认码审批
4. Worker 侧提供 Skill，指导 LLM 生成或选择脚本。
5. Java 侧提供最小审批确认码 API。
6. Worker 将 suspension 事件转为统一任务事件。
7. 前端展示审批请求并提交确认码。
8. 完成同进程 resume / reject 验证。
...
1. Worker 如何获取 Engine suspension 信息：回调、同步异常包装、还是 run manager 查询。
2. Worker/Java 调用进程内 resume/reject 的桥接方式。
3. `orderBiz` 对象门面首版在 Java 侧还是 Python 侧注册。
4. 脚本资产先落 Worker 文件系统、Java 数据库，还是 Artifact Store。
5. v1.9.1 HTTP/MCP resume endpoint 出来前，是否允许只做单进程 PoC。
