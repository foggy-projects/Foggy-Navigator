...
- [x] 完成 Worker 级 OPT-029 合同捕获脚本与基线记录：`tools/langgraph-biz-worker/scripts/opt029_timeout_recovery_capture.py` 复用 `tools/mock-llm-service` scripted endpoint 与 `delay_ms` 慢响应模拟，记录目录 `docs/version-tracker/1.3.0-SNAPSHOT/test-records/opt-029-timeout-recovery/20260518-203003-99163e/`。
- [ ] 完成真实链路治理验收：TMS 300 秒场景、断线重连、Java relay slow/read timeout、provider hang soak 仍需专门验证。
...
- [x] Java relay/session timeout tests: `mvn -pl addons/langgraph-biz-worker -am -Dtest=LanggraphStreamRelayTest,LanggraphTaskServiceTest -Dsurefire.failIfNoSpecifiedTests=false test`，`29 tests passed`.
- [x] Widget timeout/progress behavior tests: `pnpm --filter @foggy/navigator-chat-widget test -- useNavigatorChat.ux.test.ts`，`11 passed`.
- [ ] Client detach and reattach recovery tests.
- [x] LLM retry progress message tests.
- [x] Worker OPT-029 capture: `python scripts/opt029_timeout_recovery_capture.py --base-url http://localhost:3061`，基于项目 mock LLM scripted endpoint，6 个 verdict 全通过（retry progress、reason interruption、next-turn recovery、client detach、detach 后 server completed、artifact collected）。
- [ ] TMS end-to-end timeout scenario.
- [ ] Provider hang soak / leak test.
