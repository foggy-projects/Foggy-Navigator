...
- [x] Provider hang guard targeted tests: `$env:PYTHONPATH='src'; .\.venv\Scripts\python.exe -m pytest tests/test_llm_call_guard.py -q`，`3 passed`；覆盖 request timeout、retry progress、circuit open，以及底层慢调用结束后并发槽释放。
