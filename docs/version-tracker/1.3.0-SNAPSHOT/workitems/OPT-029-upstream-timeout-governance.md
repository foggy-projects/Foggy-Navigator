...
- Not yet covered: widget timeout/cancel 行为、Java session/relay timeout taxonomy、真实 TMS 300 秒链路超时收敛、provider hang soak test。
...
- [ ] 定义 task reconnect / reattach 合同：taskId、contextId、cursor、terminal signal 和历史消息补拉。
...
- [ ] Widget timeout 分支按合同展示 detach/后台继续处理状态；除用户显式 cancel 外，不调用 cancel。
- [ ] Widget 支持 reconnect/reattach 后按 cursor 补拉任务消息和最终结果。
- [ ] Java session task 记录 deadline、interruption reason、cancel reason、lastActivityAt。
...
- [ ] LLM retry/backoff 前后发出持久化 progress/retry 消息，包含 attempt、reason、remaining budget。
- [ ] UI 在 `debug` / `detail` 模式展示 retry/progress 事件，在 `simple` 模式折叠或摘要展示。
- [ ] LLM client 接入 request-level deadline/remaining budget 和统一 timeout taxonomy。
- [ ] retry 耗尽、用户取消、审批拒绝、deadline 耗尽或服务端中断后，persistent root 进入可恢复中断态，下一轮用新 task budget 并由 LLM 决定继续或终止。
- [ ] Worker 以 runtime context JSONL / journal 作为唯一事实源定位 recoverable frame，并维护单 active recoverable focus；异常出现多个 focus 时按 journal 顺序取最后一个。
- [ ] Worker 对恢复后的业务副作用执行幂等校验，避免 retry/resume 重复创建审批、工单或外部调用。
- [ ] timeout/cancel/detach/interruption 后 report、visible messages、active task 清理必须收敛。

### Stage D - Verification

- [ ] 单测覆盖 timeout taxonomy 和状态映射。
- [ ] 单测覆盖多个 active recoverable focus 时按 journal 顺序取最后一个，并将旧 focus 标记为 superseded / shelved。
- [ ] 单测覆盖 resume/retry 不重复执行业务副作用。
- [ ] 集成测试覆盖客户端断线后服务端继续运行并可重新 attach。
- [ ] 集成测试覆盖 LLM hang、LLM retry progress 消息、retry 耗尽后的可恢复中断。
- [ ] 集成测试覆盖 Worker slow、Java relay read timeout、UI wait timeout。
- [ ] E2E 覆盖 TMS 业务助手用户可见提示、取消、后台继续查看、断线恢复。
...
- [ ] 完成 interruption recovery / timeout / deadline / cancel 合同设计。
- [ ] 完成 runtime 实现。

### Testing Progress

- [x] Python LLM timeout tests: 2026-05-18 targeted `9 passed`.
- [ ] Java relay/session timeout tests.
- [ ] Widget timeout/cancel behavior tests.
- [ ] Client detach and reattach recovery tests.
- [ ] LLM retry progress message tests.
