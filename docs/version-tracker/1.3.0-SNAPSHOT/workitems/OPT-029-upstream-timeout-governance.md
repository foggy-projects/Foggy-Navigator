...
- [ ] 完成真实链路治理验收：远程 TMS OpenAPI wait-timeout/detach recovery 已通过；浏览器 UI 用户可见提示、显式 cancel 与真实断线重连仍需专门 E2E 验证。Java relay read timeout、Worker slow、UI wait timeout 和 provider hang soak 已有目标测试覆盖。
