...
采用“日期 + 一层 hash + session/context”的 canonical 目录。业务 `contextId` 必须采用 `bctx_yyyyMMdd_<hash>_<id>` 格式，其中 `<hash>` 直接作为目录分片；上游未提供时由 LangGraph BizWorker 分配，外部上游不再自行生成。新写入不再创建 task/conversation/report 索引：
...
- 新会话未携带 `contextId` 时由 LangGraph BizWorker 生成；Java 编排层可通过 Worker `/api/v1/contexts` 预分配后再绑定业务会话。
...
- completed_work:
  ...
  - LangGraph BizWorker 默认生成 `bctx_yyyyMMdd_<hash>_<uuid32>`，Worker 对业务 context 只接受 `bctx_yyyyMMdd_<hash>_<id>`，目录分片直接取 ID 内嵌 `<hash>`。
  - Java 业务任务创建先复用已有业务会话 context；新会话未传 `contextId` 时由 LangGraph BizWorker 分配后再绑定业务会话，外部上游不再需要也不建议自行生成。
...
- Python Worker 是业务侧 `contextId` 的生成方；外部上游可不传 `contextId`。
- 当前 Java 侧 `sessionId` 由会话模块/上游创建，默认是普通 UUID。
- 业务会话绑定时如果没有可复用 `contextId`，Java LangGraph launcher 会先向 BizWorker `/api/v1/contexts` 申请 `bctx_yyyyMMdd_<hash>_<uuid32>` 形式的 context，例如 `bctx_20260520_ab_0123456789abcdef0123456789abcdef`，再绑定会话。
