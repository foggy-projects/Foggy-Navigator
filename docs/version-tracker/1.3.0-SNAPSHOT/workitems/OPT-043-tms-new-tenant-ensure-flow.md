...
- 2026-05-27 TMS retest on Navi `82f0d14b`: direct admin `ensure-tenant` for tenant `110` returned HTTP 200 with `activationReady=false`, `errorCode=RUNTIME_AGENT_RESOURCE`, `missingFields=[physicalWorker.owner]`, the expected physical worker owner blocker, and owner remediation guidance. The rollback-only HTTP 500 regression is confirmed fixed on the direct Navi path.
- TMS BFF live self-healing retest for tenant `110` remains blocked by the lack of a valid TMS staff session for that tenant. Calls without a tenant `110` session return TMS auth error `code=600`, `exCode=B600`; TMS intentionally derives `trustedTenantId` only from a real staff session and does not accept `X-Trusted-Tenant-Id` header spoofing.
- The remaining live readiness blocker is operational: configure `physicalWorkerId=dev-langgraph-worker-20260504123547` as either `ownerType=UPSTREAM_SYSTEM`, `ownerId=TMS` for TMS-dedicated use, or `ownerType=PLATFORM`, `ownerId=platform` for shared runtime infrastructure. TMS' current upstream admin key does not have worker-management permission to repair this owner itself.
- 2026-05-27 operator repair: registered `physicalWorkerId=dev-langgraph-worker-20260504123547` through the SUPER_ADMIN worker identity API as shared runtime infrastructure with `ownerType=PLATFORM`, `ownerId=platform`, `workerBackend=LANGGRAPH_BIZ`, `baseUrl=http://127.0.0.1:3061`, `status=ENABLED`, and `healthStatus=HEALTHY`. A direct tenant `110` `ensure-tenant` verification with `rotateCredentials=true` then returned `activationReady=true`, `missingFields=[]`, and `blockers=[]`. Because credentials were rotated during direct verification, TMS must update its activation binding from the latest payload before relying on runtime/control credentials.
- 2026-05-27 TMS tenant `110` self-healing retest after worker owner repair: TMS BFF `ensure` returned `code=200`, `provisioned=true`, `activationReady=true`, `status=ACTIVE`, `missingFields=[]`, `blockers=[]`, and updated ACTIVE binding with rotated credentials. Readiness then passed `none/runtime/grant`, but `preflight/ask` failed with `WORKSPACE_RESOURCE - working directory tenant mismatch: 20260525-8fa8`.
- Navi now validates the working directory with the same `A2AgentResourceResolver` path used by preflight before returning `activationReady=true`. Directory tenant/owner/scope/status/binding failures are returned as structured provisioning blockers with `errorCode=WORKSPACE_RESOURCE`, `missingFields` such as `directory.tenant`, and a remediation hint. Workspace resolver methods also declare `noRollbackFor` for readiness exceptions so these caught blockers are not overwritten by HTTP 500 rollback-only failures.
- 2026-05-28 operator repair follow-up: TMS confirmed its current upstream admin key lacks `WORKING_DIRECTORY_MANAGE`, so it cannot list/init/repair `directoryId=20260525-8fa8` itself. Navi added a controlled operator/super-admin HTTP repair endpoint, `POST /api/v1/admin/working-directories/{directoryId}/repair-upstream-system`, to migrate legacy working directory ownership without direct DB edits. The endpoint stamps the directory as `tenantId=nav_tms_110`, `ownerType=UPSTREAM_SYSTEM`, `ownerId=TMS`, `workspaceScope=UPSTREAM_SYSTEM_SHARED`, upserts the root-agent directory binding, and can explicitly repair legacy root agent owner via `repairRootAgentOwner=true`.
- Real local HTTP repair for tenant `110` succeeded against the local Navigator DB. Response confirmed `directoryId=20260525-8fa8`, `tenantId=nav_tms_110`, `ownerType=UPSTREAM_SYSTEM`, `ownerId=TMS`, `workspaceScope=UPSTREAM_SYSTEM_SHARED`, `rootAgentId=tms.ops-root-agent`, `rootAgentOwnerType=UPSTREAM_SYSTEM`, `rootAgentOwnerId=TMS`, `rootAgentOwnerRepaired=true`, and `defaultDirectory=true`.
- The TMS admin-key replacement request suffix `T2Lykhp4` is still a separate credential-permission path. This repair did not require approving it because the fix was executed by Navi super-admin/operator ownership repair. A remaining UX gap is that the bootstrap approval API requires the full request code; the suffix alone is not enough for operator approval.
...
| 2026-05-27 | TMS tenant `110` direct Navi retest | Direct admin `ensure-tenant` against Navi `82f0d14b` | PASS: HTTP 200 structured not-ready response. `activationReady=false`, `errorCode=RUNTIME_AGENT_RESOURCE`, `missingFields=[physicalWorker.owner]`, blocker `physical worker owner is not configured: dev-langgraph-worker-20260504123547`, and owner remediation hint returned. |
| 2026-05-27 | TMS tenant `110` worker owner repair | SUPER_ADMIN `POST /api/v1/business-agent/worker-identities`; direct admin `ensure-tenant` verification | PASS: worker identity registered as `PLATFORM/platform`, `ENABLED/HEALTHY`; tenant `110` direct ensure returned `activationReady=true`, `missingFields=[]`, `blockers=[]` after credential rotation. |
| 2026-05-27 22:43 +08:00 | Workspace resource readiness regression | `mvn -pl business-agent-module -am "-Dtest=A2AgentResourceResolverTest,UpstreamTenantClientAppProvisioningServiceTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` | PASS: 41 tests passed. Covered working directory tenant mismatch returning `activationReady=false`, `errorCode=WORKSPACE_RESOURCE`, `missingFields=[directory.tenant]`, a workspace blocker/remediation hint, and no-rollback annotations on workspace resolver readiness exceptions. |
| 2026-05-27 22:45 +08:00 | Business Agent module full unit tests | `mvn -pl business-agent-module -am test` | PASS: 424 tests passed, 0 failures, 0 errors, 0 skipped. |
| 2026-05-27 22:46 +08:00 | Local launcher rebuild and restart | `.\start-launcher.ps1`; `Invoke-RestMethod http://localhost:8112/actuator/health` | PASS: local Navigator restarted on `http://localhost:8112`; `/actuator/health` returned `UP` with MySQL and Rabbit `UP`. |
| 2026-05-27 22:48 +08:00 | Real local HTTP smoke / mock E2E regression | `npm run typecheck`; `npx vitest run tests/02-upstream-tenant-client-app-provisioning.test.ts` in `business-agent-module/integration-tests` | PASS: TypeScript compile passed; 1 HTTP smoke test passed. The smoke now verifies fake/missing directory resources are returned as structured `WORKSPACE_RESOURCE` not-ready payloads instead of `activationReady=true`. |
| 2026-05-27 22:50 +08:00 | Full Maven reactor unit tests | `mvn test` | PASS: 240 Surefire report files, 1656 tests, 0 failures, 0 errors, 0 skipped. |
| 2026-05-28 00:07 +08:00 | Admin working directory repair targeted tests | `mvn -pl business-agent-module,user-auth-module -am "-Dtest=WorkingDirectoryAdminRepairServiceTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` | PASS: 3 tests passed. Covered directory owner migration, explicit legacy root-agent owner repair, and rejection when the caller does not opt into root-agent owner migration. |
| 2026-05-28 00:09 +08:00 | Local launcher rebuild and restart | `.\start-launcher.ps1`; `Invoke-RestMethod http://localhost:8112/actuator/health` | PASS: local Navigator restarted on `http://localhost:8112`; `/actuator/health` returned `UP` with MySQL and Rabbit `UP`. |
| 2026-05-28 00:10 +08:00 | Real local HTTP operator repair | `POST /api/v1/admin/working-directories/20260525-8fa8/repair-upstream-system` with root Bearer token and `repairRootAgentOwner=true` | PASS: HTTP response `code=200`; directory and root agent owner are both `UPSTREAM_SYSTEM/TMS`, workspace scope is `UPSTREAM_SYSTEM_SHARED`, and the directory is set as root agent default. |
| 2026-05-28 00:17 +08:00 | Full Maven reactor unit tests | `mvn test` | PASS: 241 Surefire report files, 1659 tests, 0 failures, 0 errors, 0 skipped. |
