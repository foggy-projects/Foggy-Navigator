...
- Live evidence: task `20260630-b499`, context `bctx_20260630_36_36787f40b092468e8183687a84ea0d01`, workerTask `3c9ef3e8-80c6-4061-8e6c-231c6ae0c95c`; task `20260630-af4c` confirms `TaskEvidence.structuredOutput` live lift with `source=message_content`; 2026-06-30 rerun task `20260630-0a61` and continuation task `20260630-4f73` confirm current local route still works after handoff updates; tasks `20260630-d39b` and `20260630-00d8` confirm context-only CodexBiz continuation replays scoped-home from session binding.

## Changed Surface

- `navigator-open-sdk`: OpenAPI ask top-level `allowedTools` passthrough and CLI/env parsing.
- `session-module`: explicit `providerType=codex-biz-worker` direct route when logical upstream `agentId` is present, plus context-bound scoped-home replay and conflict guards.
- `addons/codex-worker-agent`: CodexBiz scoped-home fields are projected into session provider state for later context-bound continuation.
...
- error handling and edge cases: pass. Missing token disables MCP config; stdio supports both Content-Length and JSONL; unknown tools remain errors; conflicting context-bound provider/directory/scoped-home values fail fast with `CONTEXT_WORKER_MISMATCH`.
...
- test alignment: pass. Tests cover SDK serialization, route priority, context-bound scoped-home replay/conflict guards, MCP config/token hygiene, stdio framing and OpenAPI evidence lift; live smoke covers WorkerGateway path, `TaskEvidence.structuredOutput`, and strict context-only CodexBiz continuation.
...
- Legacy CodexBiz contexts created before the 2026-06-30 scoped-home replay fix may not have `providerState` scoped-home fields. Consumers should recreate those contexts or continue once with the same scoped-home source before switching to context-only continuation.
