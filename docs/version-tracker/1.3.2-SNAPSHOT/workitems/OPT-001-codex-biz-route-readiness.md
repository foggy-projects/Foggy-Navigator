...
| Codex Biz tool parity smoke | Codex Worker / Navigator | blocked | 2026-06-29 refreshed preflight cannot create a Navigator task-scoped runtime: `owner-smoke` and `verify-agent-readiness` fail with `Agent is disabled: tms-agent-v305`; live `ask` remains intentionally skipped. |
...
| Live actor A/B smoke | PowerShell / real Codex | pass | 2026-06-29 latest local 3070 run: Actor A session `019f133f-01e5-7550-9d44-ca21fadf699a`; Actor B session `019f133f-3ed5-7d82-ae05-be0a6f7c3078`; Actor A resume reused Actor A session. |
| Navigator upstream profile inventory | Navigator CLI / local 8112 | blocked | CLI `1.0.16`; `.navigator/upstream.env` exists and is gitignored. `route list` shows 2 enabled upstream routes; `directory client-list` returns `directoryCount=0`; `model grants` returns 3 enabled grants, all `LANGGRAPH_BIZ`; no `OPENAI_CODEX` grant is available for this ClientApp. |
| BusinessFunction visibility | Navigator CLI / local 8112 | pass | `function visible` returns 65 client-visible functions, including low-risk dataset/query functions. This confirms grants are visible, but does not replace WorkerGateway invoke smoke because no task-scoped token can be issued while the Agent is disabled. |
| ContextId continuation smoke | Navigator CLI / local 8112 | blocked | Cannot produce an initial Navigator `contextId` because `ask` fails before Worker dispatch with `Agent is disabled: tms-agent-v305`; explicit wrong-provider negative smoke is deferred until an enabled gray route can create the first task. |
| `git diff --check` | workspace | pass | No whitespace errors; Git reported existing CRLF normalization warnings. |

### Next Smoke Plan

1. 在非生产 Navigator 路由上保留并继续使用原 `LANGGRAPH_BIZ` / LangBizWorker profile 作为企业应用默认路线，同时创建 `providerType=codex-biz-worker` 的灰度 Agent/WorkerPool 仅供内部调试；确认 `CODEX_BIZ_HOME_ROOT`、`CODEX_NAVIGATOR_WORKER_GATEWAY_BASE_URL`、`directoryId`、`privateAccountId/codexHomeKey` 都显式可观测。
