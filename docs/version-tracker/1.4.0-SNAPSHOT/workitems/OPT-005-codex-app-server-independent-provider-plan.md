...
- execution_mode: single-agent-cross-module

## 与版本目标的关系

本计划取代 OPT-001 后续“同一 `codex-worker` Provider 内部选择 SDK/App Server runtime”的演进方式。OPT-001 已完成的 App Server Worker 幂等接受、task store、pool、事件桥接和 instance affinity 能力继续复用；路由身份、模型后端和物理 Worker 配置按 OPT-005 重新收口。

## 总体原则

1. 先冻结命名和单一配置源，再修改路由。
2. `workerBackend` 决定 A2A Provider，Session 绑定后不重选。
3. Physical Worker 是 App Server endpoint/token 的唯一人工配置入口。
4. SDK 与 App Server 共享公共 Codex 投影代码，但不共享 Provider 身份和 fallback。
5. 不实现旧数据兼容、双读、backfill 或旧会话恢复。
...
| Owner | 职责 | 开工条件 |
|---|---|---|
| `navigator-common` | Backend 常量、独立 App Server 配置模型、共享 DTO | 命名冻结后立即开工 |
...
| `addons/claude-worker-agent` | Physical Worker 双 Codex 配置、加密、DTO、诊断、readiness | 配置模型确定后开工 |
...
| Physical Worker | `addons/claude-worker-agent/src/main/java/.../model` | App Server 配置持久化/Form/DTO | update/create | endpoint/token 唯一写源 |
| Physical Worker | `addons/claude-worker-agent/src/main/java/.../service` | 配置加密、readiness、诊断 | update | SDK/App Server 独立状态 |
...
| Worker settings | `packages/navigator-frontend/src/views/ClaudeWorkerView.vue` | 双 Codex 页签 | update | 独立保存、清除、测试连接 |
| Frontend routing | `packages/navigator-frontend/src/utils/workerBackend.ts` | Backend/Provider 映射 | update | 一一映射 |
| Existing dirty endpoint work | `addons/codex-worker-agent/.../CodexAppServerEndpoint*` | 当前未提交 endpoint CRUD | review/update/delete | 不得形成第二套人工可写配置源 |
| Existing dirty migration | `docs/migration/2026-07-12-codex-app-server-endpoints.sql` | 当前未提交 endpoint schema | review/update/delete | 本需求不要求兼容迁移 |
...
- 固定 `OPENAI_CODEX_APP_SERVER`、`codex-app-server-worker`、`CodexAppServerConfig`。
- 全仓检索 Backend/Provider 映射和 Codex runtime 判断点。
- 审阅当前未提交 Endpoint/Runtime 改动，给出保留、改造、删除清单。
- 确认 Physical Worker 配置为 endpoint/token 唯一写源。

完成门：不存在两套可写 endpoint/token 方案，所有后续步骤使用同一命名。

### Step 2：拆分 Physical Worker 配置

- Entity/Register Form/Update Form/DTO 增加独立 App Server 配置。
- SDK/App Server token 分别加密、更新和清除。
- readiness、连接测试和诊断分别实现。
- Worker capability 过滤分别判断 `codexConfig` 与 `codexAppServerConfig`。

完成门：后端测试证明两套配置互不覆盖，清除一套不影响另一套。
...
- 物理 Worker 编辑页增加 `Codex App Server` 页签。
- 两套配置独立保存、清除、测试连接和显示状态。
...
- 删除重复 Endpoint 配置 API、页面和 schema，或收敛为内部只读 runtime 状态。
...
- 不迁移旧 Session/Task/Thread/ModelConfig/Worker 配置。
